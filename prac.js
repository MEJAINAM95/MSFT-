describe('Season 1 test suite', function()
{
    ...
    {
        await browser.get('https://microsoftprod.btsmomenta.com');

        await expect(browser.getTitle()).toEqual('Momenta');
        
        await browser.driver.manage().window().maximize();

        browser.ignoreSynchronization = true;

        await element(by.id('userEmail')).sendKeys('m300@microsoft.com');
  
        await element(by.id('password')).sendKeys('ABab12$');
  
        await element(by.name('Login')).click().then(function() { browser.sleep(5000)});
  
        await element(by.name('Select your industry')).element(by.css("option[value='fc9868e7db79063f9808']")).click().then(function() {browser.sleep(10000)});
  
        await element(by.name('Continue')).click().then(function() { browser.sleep(20000)});
  
        await element(by.id('mat-radio-2')).click();
  
        await element(by.name('Continue to Next Activity')).click().then(function() {browser.sleep(5000)});

        //account code 
        await element.all(by.className('plugin-single__text')).first().click().then(function() 
        {
            await element(by.name('Continue to Next Activity')).click().then(function() {browser.sleep(5000)});
            browser.sleep(5000)
        });
        ...
        // thank you screen  
        await element(by.name('Continue to Next Activity')).click().then(function() 
        {
            browser.sleep(30000);
